...
** Last update Sun May  1 11:44:46 2016 Gauthier Cler
*/
...
void			add_sons(t_pile **pile, t_pile **tmp, t_map *map)
{
  t_pos			pos;

  if (is_possible(*pile, map, NORTH, &pos) == SUCCESS)
    add_end_save(tmp, pos, *pile, map);
  if (is_possible(*pile, map, SOUTH, &pos) == SUCCESS)
    add_end_save(tmp, pos, *pile, map);
  if (is_possible(*pile, map, EAST, &pos) == SUCCESS)
    add_end_save(tmp, pos, *pile, map);
  if (is_possible(*pile, map, WEST, &pos) == SUCCESS)
    add_end_save(tmp, pos, *pile, map);
}
