...
void plazza::ThreadPool::addJob(const std::string &fileName, const Information type) {
  {
...
size_t plazza::ThreadPool::getNbFreeWorkers() const {
  ...
  return sum;
//  return static_cast<size_t>(std::accumulate(this->_workers.begin(), this->_workers.end(), 0,
//						   [](int sum, const Worker &worker){ return sum + !worker.isBusy();}));
}
...
void plazza::ThreadPool::createWorkers(const size_t nbWorkers) {
